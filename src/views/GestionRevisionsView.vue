        ...
        <section class="sm:ml-64 custom-gradient md:p-4">
            <div class="mt-14">
                <div class="flex flex-col items-center justify-center p-4 py-10 text-center text-white">
                    <BackButton />
                    <h1 class="mb-4 text-[50px] font-bold">GESTION DE LA RÉVISION</h1>
                    ...
                    <p v-else class="mb-8 text-[24px] underline">{{ theme?.nom }}</p>
                    <button type="button" @click="showDeleteModal"
                        ...
                        Supprimer de ma révision
                    </button>
                </div>
                <RevisionGestion v-if="theme" :theme="theme" />
            </div>
            <DeleteCrud :isVisible="isDeleteModalVisible" :revisionThemeId="theme?.id" @close="hideDeleteModal"
